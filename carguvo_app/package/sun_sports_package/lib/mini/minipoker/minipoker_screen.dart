...
import 'package:sun_sports/mini/minipoker/widgets/minipoker_nohu.dart';
import 'package:sun_sports/mini/minipoker/widgets/minipoker_panel.dart';
import 'package:sun_sports/mini/minipoker_landscape/minipoker_landscape_panel.dart';
import 'package:sun_sports/shared/widgets/toast/app_toast.dart';

class MinipokerScreen extends ConsumerStatefulWidget {
  final VoidCallback? onClose;

  const MinipokerScreen({super.key, this.onClose});

  @override
  ConsumerState<MinipokerScreen> createState() => _MinipokerScreenState();
}

class _MinipokerScreenState extends ConsumerState<MinipokerScreen>
  ...
  double get _panelDesignH => _subOpen
      ? MinipokerPanel.subViewHeight
      : MinipokerPanel.panelHeight;

  bool get _subOpen => ref.read(minipokerSubViewProvider) != null;

  double get _designW => _landscape
      ? (_subOpen
          ? kMinipokerLandscapeSubViewWidth
          : kMinipokerLandscapeGameWidth)
      : _kDialogMaxWidth;

  double get _designH => _landscape
      ? (_subOpen
          ? kMinipokerLandscapeSubViewHeight
          : kMinipokerLandscapeGameHeight)
      : _panelDesignH;
  ...
  StreamSubscription<MiniPokerToast>? _toastSub;
  ...
  void initState() {
    ...
    _toastSub = ref
        .read(miniPokerStateProvider.notifier)
        .toastMessages
        .listen((toast) {
      if (!mounted) return;
      switch (toast.kind) {
        case MiniPokerToastKind.success:
          AppToast.showSuccess(context, message: toast.message);
        case MiniPokerToastKind.error:
          AppToast.showError(context, message: toast.message);
        case MiniPokerToastKind.info:
  ...
  void _minimize() {
    ...
    ref.setMiniGameExpanded(MiniGameSelection.miniPoker, expanded: false);
  }
  ...
  void _expand() {
    ...
    ref.setMiniGameExpanded(MiniGameSelection.miniPoker, expanded: true);
  }

  double _landscapeScale(double availW, double availH) {
    final byWidth = availW / _designW;
    final byHeight = availH / _designH;
  ...
  Widget build(BuildContext context) {
    ref.listen<MiniPokerState>(miniPokerStateProvider, (_, __) {});

    ref.watch(minipokerSubViewProvider);

    ref.listen<int>(
      miniGameExpandRequestProvider(MiniGameSelection.miniPoker),
      (prev, next) {
    ...
    ref.listen<int>(
      miniGameMinimizeRequestProvider(MiniGameSelection.miniPoker),
      (prev, next) {
  ...
  Widget _buildNoHu() {
    final win = ref.watch(miniPokerStateProvider.select((s) => s.jackpotWin));
    ...
    return Positioned.fill(
      child: MinipokerNoHu(
        ...
        auto: auto,
        onClose: () =>
            ref.read(miniPokerStateProvider.notifier).dismissJackpot(),
      ),
    );
  }
              ...
              return Stack(
                ...
                children: [
                  Positioned(
                    ...
                    top: clamped.dy,
                    width: w,
                    child: _expandedPanel(clamped, maxX, maxY, isMobile, h),
                  ),
                ],
              );
            },
  ...
  Widget _expandedPanel(
    ...
    final Widget content = _landscape
        ? SizedBox(
            height: h,
            child: FittedBox(
              fit: BoxFit.fill,
      ...
      children: [
        Positioned(
          ...
          child: MiniGameMinimizedHitReporter(
            id: 'minipoker',
            game: MiniGameSelection.miniPoker,
            active: true,
